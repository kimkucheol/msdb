...
try{
  ...
  $sql='select timestamp, log.name as name, item.name as item_name, period, return_item 
from lending_log, item,
(select code, name from student union select code, name from master) as log 
where lending_log.code = log.code and lending_log.item = item.id LIMIT ?, ?';
  ...
  $sql2='select timestamp, log.name as name, item.name as item_name, period, return_item 
from lending_log, item,
(select code, name from student union select code, name from master) as log 
where lending_log.code = log.code and lending_log.item = item.id LIMIT ?, 1';
...
<html>
  ...
  <body>
    <div class="setting w90">
      <h2>過去の貸出履歴<span><?php echo $_GET['to'].'～'; ?></span></h2>
      <table>
        <thead>
          <tr>
            <th>日時</th>
